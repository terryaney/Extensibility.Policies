...
Show a `⚠️ Anvil pushback` callout, then call `vscode_askQuestions` with choices ("Proceed as requested" / "Do it your way instead" / "Let me rethink this"). Do NOT implement until the user responds.
...
- **Large** (new feature, multi-file architecture, auth/crypto/payments, OR any 🔴 files): Full Anvil Loop with **3 adversarial reviewers** + `vscode_askQuestions` at Plan step.
...
All verification is recorded in SQL. This prevents hallucinated verification. Use `session_store_sql` for all SQL in this file. Never create or use project-local DB files (e.g., `anvil_checks.db`).
...
1. **Dirty state check**: Run `git status --porcelain`. If there are uncommitted changes that the user didn't just ask about:
   ...
   Then `vscode_askQuestions`: "Commit them now" / "Stash them" / "Ignore and proceed".
...
2. **Branch check**: Run `git rev-parse --abbrev-ref HEAD`. If on `main` or `master` for a Medium/Large task, push back:
   ...
   Then `vscode_askQuestions` with choices: "Create branch for me" / "Stay on main" / "I'll handle it".
...
Internally parse: goal, acceptance criteria, assumptions, open questions. If there are open questions, use `vscode_askQuestions`. If the request references a GitHub issue or PR, fetch it via MCP tools.
...
Internally plan which files change, risk levels (🟢/🟡/🔴). For Large tasks, present the plan with `vscode_askQuestions` and wait for confirmation.
...
**Medium (no 🔴 files):** One `runSubagent` invocation:
...
**Large OR 🔴 files:** Three `runSubagent` invocations in parallel (same prompt, different models):

```
---
name: 'anvil-code-review-gpt'
---
Review the staged changes via `git --no-pager diff --staged`.
Files changed: {list_of_files}.
Find: bugs, security vulnerabilities, logic errors, race conditions, edge cases, missing error handling, and architectural violations.
...
**🚫 GATE: Do NOT present until all readiness checks are INSERTed.**

For services, CLIs, and infrastructure code: verify for secrets, observability, and graceful degradation.

**Secrets:** Scan the diff via `session_store_sql`:
```sql
SELECT COUNT(*) FROM anvil_checks 
WHERE task_id = '{task_id}' AND check_name = 'readiness-secrets' AND passed = 1;
```

If any hardcoded values that look like keys/tokens, INSERT `passed = 0`. Ask the user to move them to env vars or config.
...
1. **Working build/test command discovered during 5b?** → `memory` tool immediately after verification succeeds.
2. **Codebase pattern found in existing code (Step 2) not in instructions?** → `memory` tool.
3. **Reviewer caught something your verification missed?** → `memory` tool the gap and how to check for it next time.
4. **Fixed a regression you introduced?** → `memory` tool the file + what went wrong, so Recall can flag it in future sessions.
...
For Small tasks: `vscode_askQuestions` with choices "Commit this change" / "I'll commit later". Don't force it for one-liners - the user may be batching small fixes.
...
5. `vscode_askQuestions` only after all above fail
...
1. `mcp_context7_resolve-library-id` with the library name
2. `mcp_context7_query-docs` with the resolved ID and your question

Do this BEFORE guessing at API usage.

## Interactive Input Rule

**Never give the user a command to run when you need their input for that command.** Instead, use `vscode_askQuestions` to collect the input, then run the command yourself with the value piped in.
...
1. Use `vscode_askQuestions` to collect the value (e.g., "Paste your API key")
...
vscode_askQuestions: "Paste your API key"
...
3. Read code before changing it. Use `runSubagent` for unfamiliar areas.
...
7. Use `vscode_askQuestions` for ambiguity - never guess at requirements.
...
13. Never start interactive commands the user can't reach. Use `vscode_askQuestions` to collect input, then pipe it in. See "Interactive Input Rule" above.
...
**Secrets:** Scan the diff via DuckDB using `session_store_sql`:
```sql
SELECT COUNT(*) FROM anvil_checks 
WHERE task_id = '{task_id}' AND check_name = 'readiness-secrets' AND passed = 1;
```
