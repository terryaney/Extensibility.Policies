## Plan: KatLedger MCP Auto-Bootstrap

Build a local KatLedger MCP server with SQLite-backed ledger operations, then integrate it into the existing KAT bootstrap flow so update.ps1 installs/configures it automatically for VS Code, Copilot CLI, and Claude. Use structured tool operations instead of raw SQL so statement whitelisting is largely unnecessary while still preventing destructive misuse. The current Anvil workflow still targets the `anvil_checks` table inside that general ledger.

**Steps**
1. Phase 1 - Define tool contract and runtime decisions.
2. Finalize MCP tool name (`kat/ledger`) and operation surface (`read_checks`, `insert_check`, `count_checks`, `list_checks`) with structured parameters instead of free-form SQL.
3. Decide canonical DB path (`%USERPROFILE%/.kat/KatLedger.db`) and schema initialization behavior (`CREATE TABLE IF NOT EXISTS anvil_checks`, optional `schema_version`).
...
6. Create a new local MCP server under the repo (recommended location: `AI/skills/kat-policies/mcp/KatLedger/`) with one exposed tool namespace (`kat/ledger/*`).
...
11. Add new helper script `AI/skills/kat-policies/scripts/install-katledger.ps1` following existing `-CheckOnly/-PassThru` pattern from context7/github helpers.
12. In helper, configure MCP client files for all selected clients: `%APPDATA%/Code/User/mcp.json`, `%USERPROFILE%/.copilot/mcp-config.json`, `%USERPROFILE%/.claude.json` with local stdio server entry for `kat/ledger`.
13. Extend `AI/skills/kat-policies/scripts/update.ps1` with parity detection function for `kat/ledger`, and invoke bootstrap using `Invoke-McpRemoteBootstrap` pattern (same install prompt and blocked handling style).
14. Update `AI/agents/anvil/meta.jsonc` to include required tool ID (`kat/ledger/*` or exact IDs based on final server tooling).
15. Phase 4 - Update Anvil instructions and verification behavior.
16. Update `AI/agents/anvil/body.md` copilot-vscode section to instruct ledger reads/writes through `kat/ledger` tool operations, not `session_store_sql` and not terminal duckdb CLI.
...
20. Validate bootstrap check mode: run `install-katledger.ps1 -CheckOnly -PassThru` and ensure status transitions (`needs-install` to `ok`) mirror existing helpers.
...
**Relevant files**
- `c:/BTR/Extensibility/Policies/AI/skills/kat-policies/scripts/update.ps1` - add KatLedger parity detection + bootstrap invocation.
...
- `c:/BTR/Extensibility/Policies/AI/agents/anvil/meta.jsonc` - declare `kat/ledger` tool requirement.
- `c:/BTR/Extensibility/Policies/AI/agents/anvil/body.md` - switch VS Code ledger instructions to local MCP operations.
- `c:/BTR/Extensibility/Policies/AI/skills/kat-policies/mcp/KatLedger/*` - new local MCP server implementation.
...
2. Run update sync and confirm deployment report includes KatLedger bootstrap outcome.
